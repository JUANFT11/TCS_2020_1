...
//		System.out.println("retornando: "+list);
	...
	public ResponseEntity<List<Recaudaciones>> getRecaudacionesByFilter(@RequestBody Filtro filtro) {
	 ...
	 @RequestMapping(value = "/actualizar/{id_rec}/{cod_alumno}/{id_programa}", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)	 																		  
		...
		@RequestMapping(value = "/asignar/{cod_alumno}/{numero}/{id_envio}", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)	 																		  
        public void updateRecaudacionesIDEnvio(@PathVariable("cod_alumno") String cod_alumno, @PathVariable("numero") String  numero, @PathVariable("id_envio") Integer id_envio){
            
            logger.info("> Commo00n: " + cod_alumno + " " + numero + " " + id_envio);
            Integer response = 0;
            
            try{
                 recaudacionesService.updateRecaudacionesIDEnvio(cod_alumno, numero, id_envio);
            } catch(Exception e){
                logger.error("Unexpected Exception caught. "+ e.getMessage());
            }
